import os

from qgis.core import QgsProject, QgsMapLayer


def saveLeafletMap(with_slider_range):
    print("Save leaflet")
    project = QgsProject.instance()
    dir = project.readEntry("qgis2web", "Exportfolder")[0]
    print(dir)
    root, dirs, files = next(os.walk(dir))
    subdirs = [(os.path.getctime(os.path.join(root, f)), f) for f in dirs]
    latest_subdir = max(subdirs)
    index = os.path.join(dir, latest_subdir[1], 'index.html')
    html = open(index, 'r').read()
    html = addLeafletHeader(html, with_slider_range)
    html = changeLeafletStyles(html)
    index_time = os.path.join(dir, latest_subdir[1], 'index_time.html')
    f = open(index_time, 'w')
    f.write(html)
    f.close()


def addLeafletHeader(html, with_slider_range):
    project = QgsProject.instance()
    mintime = project.readEntry("qgis2web", "Min")[0]
    maxtime = project.readEntry("qgis2web", "Max")[0]

    lines = [
        '<script src="http://code.jquery.com/jquery-1.11.1.min.js"></script>',
        ('<link rel="stylesheet"'
            'href="https://code.jquery.com'
            '/ui/1.10.2/themes/smoothness/jquery-ui.css" />'),
        ('<script src="https://code.jquery.com/ui/1.10.2/jquery-ui.js">'
            '</script>'),
        '<div style="position: fixed; top: 10px; left: 70px;">',
        '<div id="slider-range" style="width:300px"></div>',
        '<p><input id="datefrom"/>   <input id="dateto"/> </p>',
        '</div>',

        "<script>",
        "function getDateString(d) {",
        "m = d.getMonth() + 1;",
        "month = String('0' + m).slice(-2);",
        "day = String('0' + d.getDate()).slice(-2);",
        "return d.getFullYear() + '-' + month + '-' + day;",
        "}",
        "$(document).ready(function() {",
        "$( '#slider-range' ).slider({",
        "range: true," if with_slider_range else "range: false,",
        "min: new Date('" + mintime + "').getTime() / 1000,",
        "max: new Date('" + maxtime + "').getTime() / 1000,",
        "step: 86400,",
        ("values: [ new Date('" + mintime + "').getTime() / 1000, " +
            "new Date('" + maxtime + "').getTime() / 1000 ],"),

        "slide: function( event, ui ) {",
        "var from = new Date(ui.values[0] *1000);",
        "var to = new Date(ui.values[1] *1000);",
        "$( '#datefrom' ).val(getDateString(new Date(ui.values[0] *1000)));",
        ("$( '#dateto' ).val(getDateString(new Date(ui.values[1] *1000)));)"
            if with_slider_range else
            "$( '#dateto' ).val(getDateString(new Date(ui.values[0]*1000)));"),

        "setVisibility();",
        "}",
        "});",

        "var from = new Date($('#slider-range').slider('values', 0)*1000);",
        "var to = new Date($('#slider-range').slider('values', 1)*1000);",
        "$( '#datefrom' ).val(getDateString(from));",
        "$( '#dateto' ).val(getDateString(to));",
        "});",
    ]

    html = html.replace("<script>", '\n'.join(lines))
    return html


def changeLefletStyleForLayer(html, layer, layernames):
    start = html.find("function style_" + layer.name())
    flen = len("function style_") + len(layer.name())
    layeridstr = html[start + flen:start + flen + 2]
    layernames.append(layer.name() + layeridstr)
    start2 = html.find("{", start + 1)
    end = html.find("}", start + 1)
    style = html[start2 + 1:end]
    style = style.replace("return", "s = ") + "\n};"
    end = html.find("}", end + 1)
    style = "function style_" + layer.name() + layeridstr + "_0(feature) {" \
        + "\n" + style

    prop_t_from = layer.customProperty("qgis2web/Time from")
    prop_t_to = layer.customProperty("qgis2web/Time to")
    field_from = layer.fields()[int(prop_t_from) - 1].name()
    field_to = layer.fields()[int(prop_t_to) - 1].name()

    style_lines = [
        "var ftr_from = String(feature.properties." + field_from + ");",
        "var ftr_to = String(feature.properties." + field_to + ");",
        "if (ftr_from.length == 4) { ftr_from = ftr_from + '-01-01'; }",
        "if (ftr_from.length == 7) { ftr_from = ftr_from + '-01'; }",
        "if (ftr_to.length == 4) { ftr_to = ftr_to + '-01-01'; }",
        "if (ftr_to.length == 7) { ftr_to = ftr_to + '-01'; }",
        "if (",
        "(ftr_from <= $('#datefrom').val() && ftr_to <= $('#datefrom').val())",
        "||",
        "(ftr_from >= $('#dateto').val() && ftr_to >= $('#dateto').val())",
        ") {",
        " s['opacity'] = 0.0;",
        " s['fillOpacity'] = 0.0;",
        "}",
        "return s;",
        "}",

        "function setVisibility" + layer.name() + layeridstr + "() {",
        "for (var row=0; row<1000; row++) {",
        ("if (typeof(layer_" + layer.name() + layeridstr +
            "._layers[row])=='undefined') continue;"),
        ("  s = style_" + layer.name() + layeridstr + "_0(layer_" +
            layer.name() + layeridstr + "._layers[row].feature);"),
        "  layer_" + layer.name() + layeridstr + "._layers[row].setStyle(s);",
        " }",
        "}",
    ]
    style += '\n'.join(style_lines)
    html = html[:start] + style + html[end + 1:]
    start = html.find("function doPointToLayer" + layer.name())
    if start != -1:
        start = html.find("(", start + 1)
        start = html.find("(", start + 1)
        start = html.find("(", start + 1)
        html = html[:start] + "(feature" + html[start + 1:]


def changeLeafletStyles(html):
    root_node = QgsProject.instance().layerTreeRoot()
    tree_layers = root_node.findLayers()
    layerid = len(tree_layers) - 1
    layernames = []
    for tree_layer in tree_layers:
        layer = tree_layer.layer()
        if layer.type() == QgsMapLayer.VectorLayer:
            prop_t_from = layer.customProperty("qgis2web/Time from")
            prop_t_to = layer.customProperty("qgis2web/Time to")
            if prop_t_from is not None and prop_t_to is not None:
                changeLefletStyleForLayer(html, layernames,
                                          layer, prop_t_from, prop_t_to)
            layerid -= 1
    fvisibility = "function setVisibility() {\n"
    for layername in layernames:
        fvisibility += "setVisibility" + layername + "();\n"
    fvisibility += "}\n"
    html = html.replace("setBounds();", fvisibility + "setBounds();")
    return html


def saveOLMap(with_slider_range):
    print("Save OL")
    project = QgsProject.instance()
    dir = project.readEntry("qgis2web", "Exportfolder")[0]
    if dir == "":
        if os.path.isdir("/tmp/qgis2web"):
            dir = "/tmp/qgis2web"
        if os.path.isdir("C:\\TEMP\\qgis2web"):
            dir = "C:\\TEMP\\qgis2web"
    print(dir)
    root, dirs, files = next(os.walk(dir))
    subdirs = [(os.path.getctime(os.path.join(root, f)), f) for f in dirs]
    latest_subdir = max(subdirs)

    index = os.path.join(dir, latest_subdir[1], 'index.html')
    html = open(index, 'r').read()

    layernames = changeOLStyles(os.path.join(dir, latest_subdir[1], 'styles'))

    fvisibility = "function setVisibility() {\n"
    for layername in layernames:
        fvisibility += "lyr_" + layername + ".getSource().changed();\n"
    fvisibility += "}</script>\n"

    html = html.replace("_style.js", "_style_time.js")

    mintime = project.readEntry("qgis2web", "Min")[0]
    maxtime = project.readEntry("qgis2web", "Max")[0]

    lines = [
        '<head>'
        '<script src="http://code.jquery.com/jquery-1.11.1.min.js"></script>',
        '<script>',

        "function getDateString(d) {",
        "m = d.getMonth() + 1;",
        "month = String('0' + m).slice(-2);",
        "day = String('0' + d.getDate()).slice(-2);",
        "return d.getFullYear() + '-' + month + '-' + day;",
        "}",

        "$(document).ready(function() {",
        "$( '#slider-range' ).slider({",

        "range: true," if with_slider_range else "range: false,",
        "min: new Date('" + mintime + "').getTime() / 1000,",
        "max: new Date('" + maxtime + "').getTime() / 1000,",
        "step: 86400,",
        ("values: [ new Date('" + mintime + "').getTime() / 1000, "
            "new Date('" + maxtime + "').getTime() / 1000 ],"),
        "slide: function( event, ui ) {",
        "var from = new Date(ui.values[0] *1000);",
        "var to = new Date(ui.values[1] *1000);",
        "$( '#datefrom' ).val(getDateString(new Date(ui.values[0] *1000)));",
        ("$( '#dateto' ).val(getDateString(new Date(ui.values[1]*1000)));"
            if with_slider_range else
            "$( '#dateto' ).val(getDateString(new Date(ui.values[0]*1000)));"),

        "setVisibility();",
        "}",
        "});",
        "var from = new Date($('#slider-range').slider('values', 0)*1000);",
        "var to = new Date($('#slider-range').slider('values', 1)*1000);",
        "$( '#datefrom' ).val(getDateString(from));",
        "$( '#dateto' ).val(getDateString(to));",
        "});",
    ]
    header = '\n'.join(lines) + fvisibility
    html = html.replace("<head>", header)

    header += '<link rel="stylesheet" href="https://code.jquery.com' \
        + '/ui/1.10.2/themes/smoothness/jquery-ui.css" />\n'
    header += '<script src="https://code.jquery.com' \
        + '/ui/1.10.2/jquery-ui.js"></script>\n'
    header += '<div style="position: fixed; top: 10px; left: 70px;">' \
        + '<div id="slider-range" style="width:300px"></div>\n'
    header += '<p><input id="datefrom"/>   <input id="dateto"/> </p>\n'
    header += '</div>\n'
    html = html.replace("</body>", header)

    index_time = os.path.join(dir, latest_subdir[1], 'index_time.html')
    f = open(index_time, 'w')
    f.write(html)
    f.close()


def changeOLStyleForLayer(path, layer, layernames, layerid):
    stylefile = os.path.join(path, layer.name() + layerid + "_style.js")
    styletimefile = os.path.join(
        path,
        layer.name() + layerid + "_style_time.js")
    if not os.path.exists(stylefile):
        stylefile = os.path.join(path, layer.name() + "_style.js")
        styletimefile = os.path.join(path, layer.name() + "_style_time.js")
    else:
        layernames.append(layer.name() + layerid)
    if not os.path.exists(stylefile):
        stylefile = os.path.join(
            path,
            layer.name() + "_" + layerid + "_style.js")
        styletimefile = os.path.join(
            path,
            layer.name() + "_" + layerid + "_style_time.js")
        layernames.append(layer.name() + "_" + layerid)
    else:
        layernames.append(layer.name())
    style = open(stylefile, 'r').read()
    start = style.find("var style =")
    end = style.find(";", start)
    styledef = style[start + 4:end + 1]

    prop_t_from = layer.customProperty("qgis2web/Time from")
    prop_t_to = layer.customProperty("qgis2web/Time to")
    field_from = layer.fields()[int(prop_t_from) - 1].name()
    field_to = layer.fields()[int(prop_t_to) - 1].name()

    lines = [
        "var ftr_from = String(feature.get('" + field_from + "'));",
        "var ftr_to = String(feature.get('" + field_to + "'));",
        "if (ftr_from.length == 4) { ftr_from = ftr_from + '-01-01'; }",
        "if (ftr_from.length == 7) { ftr_from = ftr_from + '-01'; }",
        "if (ftr_to.length == 4) { ftr_to = ftr_to + '-01-01'; }",
        "if (ftr_to.length == 7) { ftr_to = ftr_to + '-01'; }",
        "if (",
        "(ftr_from <= $('#datefrom').val() && ftr_to <= $('#datefrom').val())",
        "||",
        "(ftr_from >= $('#dateto').val() && ftr_to >= $('#dateto').val())",
        ") {",
        styledef.replace("1.0", "0.0"),
        "}",
    ]
    style = style[:end + 1] + '\n'.join(lines) + style[end + 2:]
    f = open(styletimefile, 'w')
    f.write(style)
    f.close()


def changeOLStyles(path):
    root_node = QgsProject.instance().layerTreeRoot()
    tree_layers = root_node.findLayers()
    layerid = len(tree_layers) - 1
    layernames = []

    for tree_layer in tree_layers:
        layer = tree_layer.layer()
        if layer.type() == QgsMapLayer.VectorLayer:
            prop_t_from = layer.customProperty("qgis2web/Time from")
            prop_t_to = layer.customProperty("qgis2web/Time to")
            if prop_t_from is not None and prop_t_to is not None:
                changeOLStyleForLayer(path, layer, layernames, layerid)
            layerid -= 1
    return layernames
